...
/**
 * This abstract class implements a state space problem solver that uses a queue
 * to control its search. Subclasses must call the "setQueue" method to install
 * the particular type of queue used. Subclasses must also implement the
 * abstract "expand" method. Other protected methods can be overridden if
 * needed. This class also keeps and makes available problem solutions and
 * search statistics.
 *
 * @author tcolburn
...
public abstract class Solver {
    ...
    /**
     * Setter for this solver's queue. This must be called by a subclass.
     *
    ...
    /**
     * Expands a vertex in the state space search tree created by this solver.
     * This method is abstract and must be overridden by a subclass.
     *
     * @param u the vertex to be expanded
     * @return a list of vertices that are one move away from u in the state
     * space
     */
    public abstract List<Vertex> expand(Vertex u);

    /**
     * Adds a vertex to the queue used by this solver. May be overridden by
     * subclasses.
     *
    ...
    /**
     * Gets the current state of the problem as a vertex. May be overridden by
     * subclasses.
     *
    ...
    /**
     * Attempts to solve the problem from the current state. Search statistics
     * are recorded, and if the search is successful a solution object is
     * constructed.
     */
    public void solve() {
        statistics.clear();
     ...
     *
     * Search(s) d[s] = 0 pred[s] = null Q = {s} V = {s} while Q ≠ {} do u =
     * Remove[Q] if success(u) then return u for each v ∈ expand(u) do if v ∉ V
     * then put v in V d[v] = d[u] + 1 pred[v] = u Add(Q,v) return null
     *
     * This method must also update the queue statistics (NUM_OPS and
     * QUEUE_SIZE) and the number of CIRCULARITIES encountered during the
     * search.
     *
     * @param s a vertex representing the current state of the problem
     * @return a vertex representing a final state of the problem if one is
    ...
    private Vertex search(Vertex s) {
        ...
        Vertex key = new Vertex(123556330);
        vertices.put(s, key);
        while (!queue.isEmpty()) {
        ...
        statistics.putStat(QUEUE_SIZE, size);
        return null;
    }

    private boolean success(Vertex u) {
        return u.getData().equals(problem.getFinalState());
    }
}
