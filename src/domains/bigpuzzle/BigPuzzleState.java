 ...
 */
public class BigPuzzleState extends State {
     ...
     */
    public BigPuzzleState(int[][] tiles) {
        this.tiles = tiles;
    }

    /**
     * A nested class that represents a location within a puzzle state as a row
     * and column pair.
     */
    public static class Location {
        ...
        /**
         * Creates a string representation of this location object for debugging
         * purposes.
         *
         * @return a string representation of this location object as
         * "(row,column)"
         */
        @Override
         ...
         * @return true if this location has same row and column indexes as the
    ...
    /**
     * A puzzle state constructor that creates a new puzzle state from a given
     * puzzle state and two locations. A copy of the given state is created and
     * the contents of the two given locations are swapped. The original puzzle
     ...
     * this state
     */
    public BigPuzzleState(BigPuzzleState state, Location loc1, Location loc2) {
    ...
    public boolean equals(Object o) {
        ...
        if (o.getClass() != BigPuzzleState.class) {
            return false;
        }
        BigPuzzleState other = (BigPuzzleState) o;
        if (this == other) {
            return true;
        }
    ...
    public int hashCode() {
        int hash = 7;
        hash = 43 * hash + Arrays.deepHashCode(this.tiles);
        return hash;
    }
...
//        return sumManhattan(this, goal);
    ...
    /**
     * This method computes the number of tiles out of place in a current state
     * relative to the goal state.
     *
     * @param current the current state of the problem
     * @param goal the goal state of the problem
     * @return the number of tiles out of place in the current state relative to
     * the goal state
     */
    public static int tilesOutOfPlace(State current, State goal) {
        int count = 0;
        BigPuzzleState c = (BigPuzzleState) current;
        BigPuzzleState g = (BigPuzzleState) goal;
        for (int i = 1; i <= 15; i++) {
            if (!c.getLocation(i).equals(g.getLocation(i))) {
                count++;
            }
        }
        return count;
     ...
     * @param current the current state of the problem
     * @param goal the goal state of the problem
     * @return the sum of the distances each tile is out of place in the current
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        BigPuzzleState g = (BigPuzzleState) goal;
        for (int i = 1; i <= 15; i++) {
            Location cloc = c.getLocation(i);
            Location gloc = g.getLocation(i);
            if (!cloc.equals(gloc)) {
                int col;
                int row;
                if (cloc.getColumn() > gloc.getColumn()) {
                    col = cloc.getColumn() - gloc.getColumn();
                } else {
                    col = gloc.getColumn() - cloc.getColumn();
                }
                if (cloc.getRow() > gloc.getRow()) {
                    row = cloc.getRow() - gloc.getRow();
                } else {
                    row = gloc.getRow() - cloc.getRow();
                }
                sum = sum + col + row;
            }
        }
